...
def create_question():
    if request.method == 'POST':
        ...
        try:
            # Parse ISO string generated by JavaScript's .toISOString()
            # Handles both standard ISO and fallbacks gracefully
            if 'T' in visible_at_str and visible_at_str.endswith('Z'):
                visible_at = datetime.strptime(visible_at_str, '%Y-%m-%dT%H:%M:%S.%fZ')
                hide_at = datetime.strptime(hide_at_str, '%Y-%m-%dT%H:%M:%S.%fZ')
            else:
